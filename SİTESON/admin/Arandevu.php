...
include("baglanti2.php");
        ...
        if ($kayit_kullanici->num_rows) {
          ...
          while ($satir=$kayit_kullanici->fetch_assoc()) {
            ...
            <td>EMAİL</td>
            <td>DERS KONUSU</td> 
            <td>İSTEKLER</td></h1></tr>';
            echo '<tr>';
            echo '<td><h3>'. $satir["id"].'</td><td>'.$satir["adsoyad"].'</td><td>'.$satir["telefon"].'</td><td>'.$satir["mail"].'</td><td>'.$satir["derskonusu"].'</td><td>' .$satir["istekler"].'</h3></td>';
            echo '</tr>';
 ...
 if (isset($_POST["btn_sil"]) ) 
   { 
     ...
     $sil="delete from randevu where id='$id'";
     $calistir=mysqli_query($db,$sil);

     mysqli_close($db);
    header("location:Arandevu.php");
   }
...
<form id="form1" method="POST" action="Arandevu.php">
